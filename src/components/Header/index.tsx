...
import { Container, Title, GoRepoIcon, OutlineButton, NavLinks, NavLink } from '#components/Header/styles'
  ...
  return (
    <Container>
      <SpaceBetween>
        ...
        <MiddleAligned>
          <OutlineButton onClick={() => goTo('https://www.linkedin.com/in/aleserafiniweb/')} icon={<FaLinkedinIn />}>
            Linkedin
          </OutlineButton>
          <OutlineButton onClick={() => goTo('mailto:sera-92@hotmail.it')} icon={<GoMail />}>
            Email
          </OutlineButton>
        </MiddleAligned>
      </SpaceBetween>
      <NavLinks>
        {anchors.map((anchor) => (
          <NavLink key={anchor.target} activeClass="active" to={anchor.target} spy={true} smooth={true} offset={-16} duration={500}>
            {anchor.icon}
            {anchor.text}
          </NavLink>
        ))}
